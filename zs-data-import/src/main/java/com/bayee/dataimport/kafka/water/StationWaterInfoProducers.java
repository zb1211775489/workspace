 ...
 * @Date 2021/3/11
 **/
public class StationWaterInfoProducers {
    ...
    public StationWaterInfoProducers(String topic,long time) {
        this.topic = topic;
        this.time = time;
    }
        ...
        for (long pageIndex = 1; pageIndex <= pageNum; pageIndex++) {
            ...
            for (String s : split) {
                ...
                for (String sttp : sttps) {
                    String message = JsonUtils.processMessage2(callInterfaceService.getListStationWater(stcd, sttp, startTime, endTime));
                    if (message != null) {
